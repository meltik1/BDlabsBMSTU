...
select  lpad (i.name , length(i.name) + (i.levele)*3) from ierarchy i;
...
with tmp(rownnum1, cus, wealth) as (
    select ROW_NUMBER() over (partition by c.CUSTOMER_ID order by c.WEALTH),  c.CUSTOMER_ID , WEALTH from CUSTOMERS c
                                                                                                              join DOCUMENTS D on c.CUSTOMER_ID = D.CUSTOMER_ID join REALTY R on R.REALTY_ID = D.REALTY_ID
    order by c.CUSTOMER_ID)
select tmp.cus, tmp.wealth from tmp
where rownnum1 = 1;
